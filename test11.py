...
def process_line(data):
    ...
    if len(values) > 192:
        ...
        if plot_option.get() == "TSV Graph":
            ...
            plot_cell_average_data(cell_averages, cell_std_devs)
...
def display_data(area, new_values): 
    ...
    for row in range(15):
        for col in range(6):
            ...
            entry.insert(0, str(entry_value)+"V")
            ...
            if item_value_float > 3.95:
                ...
                entry.config(bg='#0D1B2A')

#grid banana
def create_grid(root, bg_colour):
    table_frame = tk.Frame(root, bg=bg_colour)
    table_frame.pack()
    area = []

    for col in range(6):
        label = tk.Label(table_frame, text=f"Cell Module {col+1}", bg=bg_colour, fg='white', font=('Arial',12))
        label.grid(row=0, column=col, padx=15, pady=3)
    
    for row in range(1,16):
        row_entries = []
        for col in range(6):
            entry = tk.Entry(table_frame, width=20, justify='center', state=tk.DISABLED, disabledbackground=bg_colour, fg='white', font=('Arial',13))
            entry.grid(row=row, column=col, padx=15, pady=3)
...
text_area_2 = create_grid(right_frame, DARK_BLUE)
